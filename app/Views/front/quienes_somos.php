
      <h2>BIENVENIDOS</h2>
      <p class="col-12"> Somos XPRESSO S.A tu tienda de café más confiable.
        Déjate seducir por el aroma irresistible y el sabor incomparable del café de calidad que hemos perfeccionado durante una década en la industria. En XPRESSO, fusionamos pasión y experiencia para ofrecerte la taza perfecta de café en cada sorbo.
        Desde 2014, hemos estado sirviendo la excelencia en cada grano, brindándote no solo una bebida, sino una experiencia que despierta tus sentidos y enriquece tus momentos cotidianos.
        Descubre nuestra variedad de cafés cuidadosamente seleccionados, desde los matices suaves hasta los tostados intensos, que te transportarán a un viaje sensorial único en cada taza.
        XPRESSO Sabor que despierta tus sentidos.</p>
        <img src="assets/img/cafeteria.jpeg" alt="local-fisico">

      <div class="info-myv row">
        <div class="tipo-1 col-sm-12 col-md-6 col-lg-6">
          <h3>MISIÓN</h3>
          <p> En Xpresso, nuestra misión es ofrecer una experiencia sensorial única a cada cliente, a través de la magia de nuestras cuidadosamente seleccionadas variedades de café de especialidad. Nos comprometemos a fomentar relaciones sólidas y duraderas con nuestros productores, garantizando prácticas comerciales justas y sostenibles en cada etapa del proceso. Con más de 10 años de experiencia, seguimos siendo fieles a nuestra pasión por el café, sirviendo como un puente entre las diversas culturas y comunidades cafetaleras de Colombia, Guatemala, Nicaragua, Etiopía, Kenia y más allá. En Xpresso, cada taza cuenta una historia de dedicación, artesanía y amor por el café.</p>
        </div>  
       <div class="tipo-1 col-sm-12 col-md-6 col-lg-6">
          <h3>VISIÓN</h3>
          <p>En Xpresso, nos visualizamos como los artesanos del café de especialidad, explorando los rincones más remotos del mundo en busca de los granos más exquisitos y exóticos. Nos esforzamos por ser reconocidos como la máxima expresión de calidad, pasión y excelencia en la industria del café.</p>  
        </div>